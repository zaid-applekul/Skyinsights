...
export const DatasetManager: React.FC<DatasetManagerProps> = ({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    // keep as images; structure comes from directories
    accept: {
      ...
      {activeTab === 'upload' && (
        <div className="space-y-6">
          ...
          <div
            ...
            `}
          >
            {/* CHANGE: allow directory upload so webkitRelativePath is populated */}
            <input
              {...getInputProps()}
              // eslint-disable-next-line @typescript-eslint/ban-ts-comment
              // @ts-ignore – not in React typings but supported by browsers
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <div>
                <p className="text-lg font-semibold text-gray-700 mb-2">
                  {isDragActive ? `Drop ${selectedDatasetType} folder here` : `Upload ${selectedDatasetType} Dataset`}
                </p>
                <p className="text-sm text-gray-500">
                  Drag and drop a folder with class subfolders, or click to select • JPG, PNG, WebP
                </p>
              </div>
            </div>
          </div>
