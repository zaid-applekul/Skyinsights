...
// --- Place these mappings near the top or above the component ---
const diseaseLayerRecommendationMap: Record<string, { layers: string[]; why: string }> = {
  'Apple Scab': {
    layers: ['PSRI', 'VARI', 'CIGREEN', 'GLI', 'NDMI'],
    why: 'Humidity-driven fungal disease causing chlorophyll loss and leaf wetness',
  },
  'Apple Leaf Blotch (Alternaria)': {
    layers: ['PSRI', 'VARI', 'NDRE', 'CIGREEN'],
    why: 'Stress-induced leaf damage and chlorophyll reduction',
  },
  'Powdery Mildew': {
    layers: ['NDVI', 'EVI', 'GNDVI', 'NDRE'],
    why: 'Detect reduced canopy vigor and photosynthetic efficiency',
  },
  'Brown Rot': {
    layers: ['NDMI', 'LSWI', 'TRUE COLOR'],
    why: 'Surface disease best validated visually under high humidity',
  },
  "Bull's-eye Rot": {
    layers: ['NDMI', 'LSWI', 'NDWI', 'MOISTURE-INDEX'],
    why: 'Waterlogging and excessive root-zone moisture',
  },
  'Sooty Blotch': {
    layers: ['NDMI', 'LSWI', 'TRUE COLOR'],
    why: 'Surface disease best validated visually under high humidity',
  ...
  'Fireblight': {
    layers: ['PSRI', 'NDRE', 'MCARI', 'TCARI'],
    why: 'Rapid tissue damage and pigment breakdown',
  },
};

const pestLayerRecommendationMap: Record<string, { layers: string[]; why: string }> = {
  'Fruit Fly': {
    layers: ['NDVI', 'TRUE COLOR'],
    why: 'Indirect canopy stress + visual fruit damage',
  },
  'Tent Caterpillar': {
    layers: ['NDVI', 'EVI', 'TRUE COLOR'],
    why: 'Defoliation and canopy thinning',
  },
  'Fruit Borer': {
    layers: ['NDVI', 'NDRE', 'TRUE COLOR'],
    why: 'Fruit stress and visual confirmation',
  },
  'European Red Mite': {
    layers: ['NDVI', 'NDRE', 'PSRI'],
    why: 'Dry stress and chlorophyll degradation',
  },
  'San José Scale': {
    layers: ['TRUE COLOR', 'FALSE COLOR'],
    why: 'Bark and branch pest requiring visual inspection',
  },
  'Leaf Miner': {
    layers: ['VARI', 'PSRI', 'TRUE COLOR'],
    why: 'Leaf tissue damage and discoloration',
  },
  'Woolly Apple Aphid': {
    layers: ['NDMI', 'NDVI', 'GNDVI'],
    why: 'Root + shoot stress linked to moisture',
  },
  'Green Apple Aphid': {
    layers: ['NDRE', 'GNDVI', 'NDVI'],
    why: 'Nitrogen-related lush growth patterns',
  },
};


// --- Helper component ---
type RecommendedLayersBoxProps = {
  detected?: string;
  mapping: Record<string, { layers: string[]; why: string }>;
  onLayerSelect?: (layerId: string) => void; // Optional callback for fetching layer
};

function RecommendedLayersBox({ detected, mapping, onLayerSelect }: RecommendedLayersBoxProps) {
  const norm =
    detected?.replace(/\s*\(.*?\)/, '')
      .replace(/[\u2011\u2012\u2013\u2014\u2015]/g, '-')
      .replace(/\s*-\s*/g, '-')
      .trim() ?? '';
  const rec = mapping[norm] || null;
  ...
  if (!rec) {
    return (
      <div className="text-xs text-gray-500">
        No recommended layers for this issue.
      </div>
    );
  }
  ...
  useEffect(() => {
    if ('speechSynthesis' in window) {
      ...
      const populateVoices = () => {
        ...
        // Check for Urdu, Hindi, or English voices
        const hasUrdu = allVoices.some(
          v =>
            (v.lang && v.lang.toLowerCase().startsWith('ur')) ||
            (v.name && v.name.toLowerCase().includes('urdu'))
        );
        const hasHindi = allVoices.some(
          v =>
            (v.lang && v.lang.toLowerCase().startsWith('hi')) ||
            (v.name && v.name.toLowerCase().includes('hindi'))
        );
        const hasEnglish = allVoices.some(
          v =>
            (v.lang && v.lang.toLowerCase().startsWith('en')) ||
            (v.name && v.name.toLowerCase().includes('english'))
        );

        if (!hasUrdu && !hasHindi && !hasEnglish) {
          setVoiceWarning(
            '⚠️ No suitable Urdu, Hindi, or English voice found on this device. Speech may not work or may use an unexpected accent.'
          );
        } else if (!hasUrdu && !hasHindi) {
          setVoiceWarning(
            '⚠️ No Urdu or Hindi voice found.'
          );
        } else if (!hasUrdu) {
          setVoiceWarning(
            '⚠️ No Urdu voice found.Using English Instead'
          );
        } else {
...
const handlePlanetAutoFill = (climate: any) => {
  ...
  // Helper to check if Urdu voice is available
  const hasUrduVoice = voices.some(
    v =>
      (v.lang && v.lang.toLowerCase().startsWith('ur')) ||
      (v.name && v.name.toLowerCase().includes('urdu'))
  );

  // Decide preferred speech language once (Urdu → English)
  const preferredLang: 'ur' | 'en' =
    hasUrduVoice ? 'ur' : 'en';

  // Pick a matching voice safely
  const selectedVoice =
    Array.isArray(voices)
      ? voices.find(v => v.lang?.toLowerCase().startsWith(preferredLang)) || voices[0]
      : undefined;

  // Always provide a language code for speech (use 'ur-PK' for Urdu)
  const langCode = selectedVoice?.lang || (preferredLang === 'ur' ? 'ur-PK' : 'en-US');

  // Updated summary function to accept language
  const getSummary = (lang: 'ur' | 'en') => {
    ...
    return results.slice(0, 3).map((r) => {
      ...
      if (lang === 'ur') {
        const name =
          view === 'Diseases'
            ? diseaseUrduNames[r.name] ||
              diseasePreventionGuide?.[normName]?.ur?.[0] ||
              r.name
            : pestUrduNames[r.name] ||
              pestPreventionGuide?.[normName]?.ur?.[0] ||
              r.name;
        ...
        const tipsText =
          tips && tips.length > 0
            ? tips.join('، ')
            : 'کوئی احتیاطی تدابیر دستیاب نہیں۔';

        return `پیش گوئی شدہ ${view === 'Diseases' ? 'بیماری' : 'کیڑا'}: ${name}۔ احتیاطی تدابیر: ${tipsText}`;
      }

      // English fallback
      const name =
        view === 'Diseases'
          ? diseaseEnglishNames[r.name] || r.name
          : pestEnglishNames[r.name] || r.name;
      ...
      return `Predicted ${view === 'Diseases' ? 'disease' : 'pest'}: ${name}. Prevention: ${tipsText}`;
    }).join(' ');
  };
    ...
    🚀 Satellite Live Plus
      ...
      <div className={`grid gap-4 flex-1 overflow-hidden ${showClimateForm ? 'grid-cols-1 lg:grid-cols-[1fr_1.6fr]' : 'grid-cols-1'}`}>
        ...
        <div className="flex flex-col gap-3 overflow-y-auto pr-2">
          ...
          {/* 🧪 Recommended Satellite Layers to Inspect - RIGHT BELOW FARM SCORE */}
          {results.length > 0 && (
            <div className="mt-4 bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <button
                className="w-full flex items-center justify-between text-xs font-semibold p-2 bg-gray-100 hover:bg-gray-200 rounded transition-all mb-2"
                onClick={() => setShowRecommended((v) => !v)}
              >
                <span>🧪 Recommended Satellite Layers to Inspect</span>
                <span className={`transform transition-transform ${showRecommended ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {showRecommended && (
                <RecommendedLayersBox
                  detected={
                    results[0]?.name?.replace(/\s*\(.*?\)/, '').replace(/[\u2011\u2012\u2013\u2014\u2015]/g, '-').replace(/\s*-\s*/g, '-').trim()
                  }
                  mapping={view === 'Diseases' ? diseaseLayerRecommendationMap : pestLayerRecommendationMap}
                />
              )}
            </div>
          )}
          ...
          <PlanetMapViewer
            ...
            recommendedLayers={
              results.length > 0
                ? Array.from(new Set(
                    results
                      .slice(0, 2) // Only top two
                      .flatMap(r =>
                        (view === 'Diseases'
                          ? diseaseLayerRecommendationMap[r.name]?.layers
                          : pestLayerRecommendationMap[r.name]?.layers
                        ) || []
                      )
                  ))
                : []
      ...
      {Array.isArray(results) && results.length > 0 ? (
        <div className="mt-4 pt-4 border-t border-gray-200">
          {/* Speaker Icon and Voice Warning */}
          <div className="flex flex-col items-start mb-2">
            {voiceWarning && (
              <div className="mb-1 text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 rounded px-2 py-1">
                {voiceWarning}
              </div>
            )}
            <Speech
              text={getSummary(preferredLang)}
              ...
              displayText={
                <span className="mr-2 text-green-700 hover:text-green-900 cursor-pointer flex items-center">
                  ...
                  <span className="text-xs text-gray-500 ml-1">
                    {preferredLang === 'ur' ? 'نتائج سنیں' : 'Listen to results'}
                  </span>
                </span>
              }
            />
...
const diseaseEnglishNames: Record<string, string> = {
  'Apple Scab': 'Apple Scab',
  'Apple Leaf Blotch (Alternaria)': 'Apple Leaf Blotch (Alternaria)',
  'Powdery Mildew': 'Powdery Mildew',
  'Brown Rot': 'Brown Rot',
  "Bull's‑eye Rot": "Bull's-eye Rot",
  'Sooty Blotch': 'Sooty Blotch',
  'Flyspeck': 'Flyspeck',
  'Collar / Root Rot': 'Collar / Root Rot',
  'Fireblight': 'Fireblight',
};

const pestEnglishNames: Record<string, string> = {
  'Fruit Fly': 'Fruit Fly',
  'Tent Caterpillar': 'Tent Caterpillar',
  'Fruit Borer': 'Fruit Borer',
  'European Red Mite': 'European Red Mite',
  'San José Scale': 'San José Scale',
  'Leaf Miner': 'Leaf Miner',
  'Woolly Apple Aphid': 'Woolly Apple Aphid',
  'Green Apple Aphid': 'Green Apple Aphid',
};
...
const pestUrduNames: Record<string, string> = {
  ...
  'Woolly Apple Aphid': 'اون والا سیب افڈ',
  'Green Apple Aphid': 'سبز سیب افڈ',
};
