...
y = 50; // move below logo
  ...
  // Urdu voice selection (fallback to default if not found)
  const urduVoice =
    voices?.find(
      (v) =>
  ...
  // Urdu translation for risk levels
  const urduRiskLevel: Record<string, string> = {
    High: 'زیادہ خطرہ',
    Medium: 'درمیانہ خطرہ',
    Low: 'کم خطرہ',
  };

  // Urdu translation for "Score"
  const urduScore = 'اسکور';

  // Urdu translation for "disease" and "pest"
  const urduDisease = 'بیماری';
  const urduPest = 'کیڑا';

  // Urdu translation for "Prevention Tips"
  const urduPrevention = 'احتیاطی تدابیر';

  // Compose Urdu summary for current results (name, risk, score, prevention)
  const getUrduSummary = () => {
    ...
    return results.slice(0, 3).map((r, idx) => {
      const name = r.name;
      const level = urduRiskLevel[r.level] || r.level;
      const score = Math.round(r.score);
      const preventionTips =
        view === 'Diseases'
          ? diseasePreventionGuide?.[r.name]?.ur ?? []
          : pestPreventionGuide?.[r.name]?.ur ?? [];
      let tipsText = '';
      if (preventionTips.length > 0) {
        tipsText =
          `${urduPrevention}: ` +
          preventionTips.slice(0, 2).join('، ') +
          (preventionTips.length > 2 ? `، اور مزید تدابیر موجود ہیں۔` : '');
      }
      return `${idx + 1}. ${name} — ${view === 'Diseases' ? urduDisease : urduPest}، خطرہ: ${level}، ${urduScore}: ${score} فیصد۔ ${tipsText}`;
    }).join(' ');
  };
      ...
      {Array.isArray(results) && results.length > 0 ? (
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          <div className="flex items-center mb-2">
            <button
              type="button"
              aria-label="Speak results in Urdu"
            ...
            <span className="text-xs text-gray-500">نتائج سنیں</span>
          </div>
          {/* Risk Summary */}
